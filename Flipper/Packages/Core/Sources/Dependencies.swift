...
public final class Dependencies: ObservableObject, Sendable {
    ...
    // Raw device APIs, exposed for the Nikita assistant's device bridge. These
    // are the same protocol instances the rest of Core uses; publishing them
    // lets the app target build a NikitaDeviceBridge without reaching into the
    // private `api` struct.
    public var nikitaStorage: StorageAPI { api.storage }
    public var nikitaApplication: ApplicationAPI { api.application }
    public var nikitaGUI: GUIAPI { api.gui }
    public var nikitaSystem: SystemAPI { api.system }
    public var nikitaDesktop: DesktopAPI { api.desktop }
}
